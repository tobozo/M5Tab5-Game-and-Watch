...
static void gw_system_sound_melody(unsigned char data)
{
	...
	if (gw_audio_buffer_idx < sizeof(gw_audio_buffer)) {
		...
		// TODO: notify audio buffer full
		// printf("Audio buffer overflow at index %d\n", gw_audio_buffer_idx );
	}
}
		...
		if (((io_S >> Sx) & 0x1) != 0)
		{
			...
			else
			{
				...
				if ( key_soft_value != 0 )
				{
					...
					// un-removed existing soft key code ... because why were they removed in the first place ?
					...
					if ((gw_keyboard[Sx] & GW_MASK_K2) == 0x4000 && (gw_keyboard[Sx] & GW_MASK_K3) == 0x800000 ) {
